...
public class eclat { 
  ...
  */ 
 public  Void Itemsets (String output, String database, double minsupp, 
 ...
 // this.minsupRelative = (int) Math.ceil(minsupp * database.size()); 
  ...
  // matrix = new TriangularMatrix(maxItemId + 1); 
   ...
   // and update the triangular matrix. 
    ...
    Map<Integer, Set<Integer>> itemset = null;
...
loopJ:  for(int j=i+1; j < frequentItems.size(); j++) { 
    ...
    if(useTriangularMatrixOptimization) { 
     ...
     int support = getSupportForItems(itemI, itemJ); 
     // if not frequent 
     if (support < minsupRelative) { 
  ...
  return Itemsets(null, null, 0, false);  
 } 
 
 private int getSupportForItems(Integer itemI, int itemJ) {
...
/**
  ...
  */ 
 private int calculateSupportSingleItems(String  database, 
  ...
  int item = 0;{ 
    ...
    if (set == null) { 
     set = new HashSet<Integer>();     mapItemCount.put(item, set); 
...
//    } 
 ...
 public void printStats() { 
  ...
  Itemset database = null;
System.out.println(" Transactions count from database : "+ database.size()); 
  System.out.println(" Frequent itemsets count : " + itemsetCount); 
  System.out.println(" Total time ~ " + temps + " ms"); 
  //System.out.println(" Maximum memory usage : " + MemoryLogger.getInstance() + " mb"); 
  System.out.println("==================================================="); 
 } 
